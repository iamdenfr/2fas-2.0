...
async function fetchWeatherDataForCity(cityId) {
    try {
        ...
        if (users.length > 0) {
            ...
            if (response.status !== 200) {
                if (response.status === 401) {
                    console.error(`Error fetching weather data for City ID ${cityId}: Invalid API key`);
                } else {
        ...
        if (error.code === 'ERR_SOCKET_CONNECTION_TIMEOUT') {
            console.error(`Error fetching weather data for City ID ${cityId}: Request timeout`);
        } else {
